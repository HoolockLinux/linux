				  ...
				  struct nvme_command *cmd)
{
	...
	if (anv->hw->has_lsq_nvmmu) {
		...
		if (q->sq_head == anv->hw->max_queue_depth)
			q->sq_head = 0;
	}
	...
	if (anv->hw->has_lsq_nvmmu || q->is_adminq)
		...
		memcpy(&q->sqes[tag*2], cmd, sizeof(*cmd));
				  ...
				  struct apple_nvme_queue *q)
{
	...
	/*
	 * On Apple A11, the NVMe Submission Queue Entry Size (IOSQES) is fixed
	 * at 7, but Linux’s default NVME_NVM_IOSQES is defined as 6. Invoking
	 * nvmet_cc_iosqes(6) does not alter the controller’s behavior on
	 * these devices.
	 * To reconcile this difference, compute a size multiplier as:
	 *     1 << (DEVICE_IOSQES - NVME_NVM_IOSQES)
	 * where DEVICE_IOSQES reflects the hardware-enforced value (7 on A11).
	 */
	if (!anv->hw->has_lsq_nvmmu)
		sq_depth_multiplier = 2;

	q->sqes = dmam_alloc_coherent(anv->dev,
				      sq_depth_multiplier * depth *
